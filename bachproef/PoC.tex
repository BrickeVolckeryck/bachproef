...
\begin{itemize}
    ...
    \item \textbf{Uitgaand verkeer (Check output):} De parser isoleert het veld \texttt{\$.text} uit de respons van de chatbot voordat deze de edge verlaat, eveneens gecapped op 2048 karakters.
\end{itemize}
...
Wanneer een gebruiker bijvoorbeeld een prompt in een vreemde taal verstuurt en daarmee de regel \texttt{LLM input includes a non-allowed language} triggert, verbreekt Akamai niet simpelweg de verbinding, maar serveert het de edge-respons uit Figuur \ref{fig:akamai_modify}:
...
Deze specifieke inrichting garandeert dat de javascript-logica in de index.html van de chatbot-frontend de exacte reden van de blokkade direct kan parsen en op een gebruiksvriendelijke manier aan de eindgebruiker kan tonen.
...
De configuratie van de digitale assistent vormt een cruciaal onderdeel van de beveiligingsketen. In de standaardconfiguratie maakt Rasa vaak gebruik van WebSockets (via Socket.IO) voor real-time communicatie. Tijdens de initiële integratietests werd echter vastgesteld dat de specifieke protocol-framing van Socket.IO (bijvoorbeeld de numerieke 42 prefix voor berichten) de JSON-structuur 'vervuilt'. Dit maakte het voor de Akamai Firewall for AI onmogelijk om de payload semantisch te parsen en de geconfigureerde JSONPath-inspectie op het \texttt{\$.message} veld toe te passen.\\
...
De ActiveGate VM maakt via het beveiligde \textit{Akamai EdgeGrid}-authenticatieprotocol rechtstreeks verbinding met de SIEM API van Akamai (zie Figuur \ref{fig:dt_akamai_config}). De extensie op de VM pollt dit cloud-eindpunt periodiek om nieuwe beveiligingslogs op te halen. Door de configuratie van het unieke SIEM-configuratie-ID van de chatbot, wordt gegarandeerd dat uitsluitend de relevante security-events van de Firewall for AI worden binnengehaald en geanalyseerd.
...
Wanneer er binnen de tabel doorklikt naar de details van een specifiek incident, ontsluit Dynatrace de volledige forensische drill-down weergave (zie Figuur \ref{fig:dt_finding_detail}). Omdat de ActiveGate de exacte metadata van de blokkade synchroniseert met de interne applicatie-telemetrie, krijgt de beheerder in dit venster direct inzage in de exacte tekst of invoerstring die door de Firewall for AI is tegengehouden. Dit maakt een snelle en diepgaande root-cause analyse mogelijk zonder dat er handmatig logs uit verschillende systemen gecorreleerd hoeven te worden.
...
\begin{itemize}
    ...
    Wanneer specifieke backend-microservices of Custom Actions expliciete sleutel-waardeparen exporteren, targets de transformator deze attributen met chirurgische precisie. Indien er gestructureerde velden zoals \texttt{rasa.password}, \texttt{rasa.card\_longNum} of \texttt{rasa.email} in de span-metadata aanwezig zijn, worden de werkelijke waarden direct geëvalueerd en overschreven met statische indicators zoals \texttt{[REDACTED]} of \texttt{[REDACTED\_CC]}.\\
    
    Het attribuut \texttt{rasa.card\_ccv} vormt hierbij een voorbeeld van een \textit{failsafe} binnen een \textit{Defense-in-Depth} strategie. Binnen de applicatielogica van de Custom Actions (\texttt{actions.py}) is reeds ingesteld dat de CVC-waarde bij de registratie van de span handmatig wordt aangepast naar \texttt{[REDACTED\_CVC]}. De transformatieregel binnen de OpenTelemetry Collector dientr hier als de ultieme achtervang: mocht de applicatielaag door een code-wijziging falen in het maskeren, dan garandeert de netwerkperimeter dat de gevoelige driecijferige code alsnog onherroepelijk wordt gezuiverd.
    ...
    \begin{enumerate}
        ...
        \item \textit{Financiële Transactiebeveiliging:} Ketenreeksen van 13 tot 16 opeenvolgende cijfers (met of zonder spaties en koppeltekens) worden gedetecteerd als potentiële creditcardnummers en omgezet naar \texttt{[REDACTED\_CC]}.
        \item \textit{Context-gevoelige CVC/CVV Filters:} Om te voorkomen dat willekeurige getallen (zoals bestelhoeveelheden) ten onrechte worden gemaskeerd, zoekt de filter specifiek naar contextwoorden. De expressie \texttt{(?i)(cvv|cvc|security code)\textbackslash \textbackslash s*:?\textbackslash \textbackslash s*\textbackslash \textbackslash d\{3,4\}} zorgt ervoor dat uitsluitend combinaties in de directe nabijheid van deze trefwoorden worden gecensureerd tot \texttt{[REDACTED\_CVC]}.
    ...
    processors:
      transform:
        ...
        trace_statements:
          - context: span
            statements:
              # --- THE SNIPER: Wipe known sensitive attributes entirely ---
              ...
              # --- THE SHOTGUN: Scrub the raw chat history (INPUT) ---
              ...
              # --- THE SHOTGUN: Scrub the raw chat history (OUTPUT) ---
...
De effectiefste verdedigingslinie voor een digitale assistent is het voorkomen dat malafide of ongewenste prompts het taalmodel of de backend überhaupt bereiken. De Firewall dient hier als de absolute poortwachter. Op basis van de regels geïllustreerd in sectie \ref{sec:ai_firewall_rules}) (zoals Toxic content en Code detected), is een iteratief Bash-script opgesteld dat vier specifieke aanvalsvectoren op de input simuleert:
...
In de centrale logviewer van het dashboard (zie Figuur \ref{fig:dt_ai_observability}) is te zien hoe alle interacties op het endpoint \texttt{/webhook/secure\_rest} binnenkomen voor de \texttt{rasa-core} service. Dynatrace vangt hier de OpenTelemetry-velden op. Per chatbericht wordt de exacte starttijd, de verwerkingstijd (duration) en de status bijgehouden. \\

Wat deze weergave sterk maakt, is dat de invoer van de gebruiker (\texttt{Gen ai input messages}) Dit zorgt ervoor dat reguliere berichten (zoals \textit{"Good I like to buy some socks"} of intents zoals \texttt{/greet}) volledig leesbaar zijn voor debugging, terwijl de privacygevoelige formulieren via onze eerdere masking-regels netjes worden afgeschermd.
...
Wanneer een beheerder in de tabel doorklikt op een specifieke interactie, opent Dynatrace de volledige \textit{Distributed Trace} onder het tabblad 'Prompts' (zie Figuur \ref{fig:dt_trace_prompts}). Dit venster legt de exacte route en de responstijden bloot die een chatbericht aflegt door de hybride infrastructuur.\\
...
Hier is te zien dat de \texttt{rasa.username} correct is geregistreerd als "user" ten behoeve van de aansprakelijkheid en tracking. Daarnaast toont het platform de unieke \texttt{rasa.message\_id} en de geclassificeerde intentie (\texttt{add\_to\_cart}). De betrouwbaarheid van de NLU-engine wordt direct onderbouwd door de \texttt{rasa.intent.confidence} score, die in dit geval op \texttt{0.9904} ligt. Dit bewijst dat de chatbot met een zekerheid van 99\% heeft bepaald dat de gebruiker een product wilde toevoegen. De chatbot is voorzien van een fallback-mechanisme dat in werking treedt wanneer de confidence score onder 0,7 zakt.
...
\begin{enumerate}
    \item \textbf{Functionele Applicatiedata (Volledige naleving):} De backend-architectuur (de Java/Helidon microservices van de \texttt{Sockshop}) is volledig ingericht op het recht op gegevenswissing. Indien een gebruiker een legitiem verzoek indient, is het platform in staat om het volledige gebruikersprofiel te verwijderen.
