...
Wanneer uit deze status blijkt dat de gebruiker zich in een actieve, gevoelige gesprekslus bevindt—zoals het invullen van een creditcardformulier (\texttt{card\_form}) of registratiegegevens (\texttt{register\_form})—classificeert het kanaal de invoer als potentieel privacygevoelig. In de Dynatrace-telemetrie wordt de invoertekst dan preventief gemaskeerd en overschreven met de string \texttt{[REDACTED\_DUE\_TO\_ACTIVE\_FORM]}.

Op identieke wijze controleert het kanaal de uitgaande responsen (\texttt{collector.messages}) op gevoelige profieldata of adresbevestigingen, en maskeert deze in de uitgaande OpenTelemetry-attributen. In Listing~\ref{lst:custom_channel} is de exacte Python-implementatie van deze veilige REST-architectuur weergegeven.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{img/ai_observabilty.jpg}
